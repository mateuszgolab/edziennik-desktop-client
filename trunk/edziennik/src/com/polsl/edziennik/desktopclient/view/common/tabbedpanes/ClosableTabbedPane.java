package com.polsl.edziennik.desktopclient.view.common.tabbedpanes;
...
public class ClosableTabbedPane extends JideTabbedPane {
	...
	private class TabCloseUI implements MouseListener, MouseMotionListener {
		...
		private void controlCursor() {
			if (tabbedPane.getTabCount() > 0)
				if (closeUnderMouse(meX, meY)) {
					...
					if (selectedTab > -1)
						tabbedPane.setToolTipTextAt(selectedTab, "Zamknij " + tabbedPane.getTitleAt(selectedTab));
				} else {
					tabbedPane.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
					if (selectedTab > -1) tabbedPane.setToolTipTextAt(selectedTab, title);
				}
		}
		...
		private boolean isUnderMouse(int x, int y) {
			if (Math.abs(x - meX) < width && Math.abs(y - meY) < height) return true;
			return false;
		}
